...
namespace {
    ...
    const std::vector<std::string> FunctionsToInsert = {"checkTrace", "report", "cmpstr",
                                                        "initRandom"};
    ...
    void StateProtectorPass::parseConfig(std::string fileName) {
        ...
        if (functionsToProtect.empty() || inputProgram.empty() || syminputC.empty() ||
            syminputBC.empty() || connectivity <= 0) {
    ...
    bool StateProtectorPass::doInitialization(Module &M) {
        ...
        pureFunctionsTestCases =
            generateTestCasesForFunctions(pureFunctions, maxFunctionNameLength);
    ...
    Graph StateProtectorPass::createCheckerNetwork(std::vector<Function *> allFunctions,
        ...
        for (auto function : allFunctions) {
            ...
            if (std::find(pureFunctions.begin(), pureFunctions.end(), function) !=
                pureFunctions.end()) {
        ...
        if (connectivity > checkerFunctions.size() - 1) {
            errs() << WARNING << "The specified connectivity " << connectivity
        ...
        for (auto checkee : checkeeFunctions) {
            ...
            while (out < connectivity) {
                ...
                // If the edge was added increase the counter and repeat until connectivity is
                // reached
    ...
    Value *StateProtectorPass::createLLVMValue(IRBuilder<> *builder, std::string value,
                                               Type *type) {
        ...
        if (type->isPointerTy()) {
            ...
            // Integer types can be of length 8 bit (char), 16 bit (short), 32 bit (int), 64 bit
            // (long) and 128 bit (long long)
            ...
            // Floating point types can be of length 32 bit (float), 64 bit (double) and 80/128 bit
            // (long double)
            if (type->isFloatTy()) {
    ...
    void StateProtectorPass::insertProtect(Module &M, Function *checker, Function *checkee) {
        ...
        BasicBlock *reportBlock =
            BasicBlock::Create(M.getContext(), "reportBlock", checker, firstBasicBlock);
        ...
        for (unsigned int j = 0; j < pureFunctionsTestCases[checkee->getName()].size(); j++) {
            BasicBlock *checkBlock =
                BasicBlock::Create(M.getContext(), "funcNotOnStack", checker, reportBlock);
            funcNotOnStack.push_back(checkBlock);
        }

        // Create basic block to check if the checkee is on the call stack -> prevent circles
        BasicBlock *funcOnStackTest =
            BasicBlock::Create(M.getContext(), "funcOnStack", checker, funcNotOnStack[0]);
        ...
        // Call the checkee once for each test case, compare the results and jump to the report
        // function if necessar<
        for (unsigned int j = 0; j < pureFunctionsTestCases[checkee->getName()].size(); j++) {
            ...
            for (auto &argument : checkee->getArgumentList()) {
                std::string testParameter =
                    pureFunctionsTestCases[checkee->getName()][std::to_string(j)][PARAMETER][i]
                        .asString();
            ...
            std::string testResult =
                pureFunctionsTestCases[checkee->getName()][std::to_string(j)][RESULT].asString();
            ...
            if (checkee->getReturnType()->isIntegerTy()) {
                ...
                Constant *strcmpFunction = M.getOrInsertFunction(
                    FunctionsToInsert[cmpstr],
            ...
            // Call the report function if a check was negative or jump to the next basic block
            // (checker/ original function) if the test was positive
        ...
        // Insert a call to the report function into the creared basic block that can be called from
        // checker functions
        builder.SetInsertPoint(reportBlock);
        Constant *reportFunction =
            M.getOrInsertFunction(FunctionsToInsert[report], FunctionType::get(voidTy, false));
